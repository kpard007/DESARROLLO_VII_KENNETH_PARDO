...
$texto = "El código postal es 75626";
...
$tweet = "Sígueme en @ken_johan13 para más contenido!";
...
function validarCedulaPanama($cedula) {
    $patron = "/^\d{1,2}-\d{3}-\d{3,4}$/";
    return preg_match($patron, $cedula);
}

// validar placa de un carro
function validarPlacaPanama($placa) {
    $patron = "/^[A-Z]{1}\d{3,6}$/";
...
$placa = "A123456";
echo "</br>";
echo "¿Número de cédula válido? " . (validarCedulaPanama($cedula) ? "Sí" : "No") . "</br>";
echo "¿Número de placa válido? " . (validarPlacaPanama($placa) ? "Sí" : "No") . "</br>";
...
//////////////////////////////////////////////////////////////////////////
function extraerInfoPersonal($texto) {
    $patron = "/Nombre: ([^,]+), Edad: (\d+), Mascota: ([^,]+)/";
    if (preg_match($patron, $texto, $coincidencias)) {
        return [
            'nombre' => $coincidencias[1],
            'edad' => $coincidencias[2],
            'mascota' => $coincidencias[3]
        ];
    }
    return null;
}
...
// ejemplo
$texto = "Nombre: Carlos, Edad: 25, Mascota: Max";
$info = extraerInfoPersonal($texto);
echo "</br>";
if ($info) {
    echo "Información extraída:</br>";
    echo "Nombre: {$info['nombre']}</br>";
    echo "Edad: {$info['edad']}</br>";
    echo "Mascota: {$info['mascota']}</br>";
} else {
    echo "No se pudo extraer la información</br>";
}
