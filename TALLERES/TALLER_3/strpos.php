...
$texto = "Hola, mundo extraordinario!";
$posicion = strpos($texto, "extraordinario");
echo "La palabra 'extraordinario' comienza en la posición: $posicion</br>";

// Búsqueda que no encuentra resultados
$busqueda = strpos($texto, "CSS");
if ($busqueda === false) {
    echo "La palabra 'CSS' no se encontró en el texto.</br>";
}

$text2 ="Contra amigos felices";
$encuentra = strpos ($text2, "amigos felices");
echo "La contranseña no puede contener los valores a partir de la posicion: $encuentra </br>";
...
$emails = ["kenneth1@gmail.com","kenneth2@google.com","kenneth3@yahoo.com","kenneth4@outlook.com"];

function emailPermitido($email, $dominiosPermitidos) {
    foreach ($dominiosPermitidos as $dominio) {
        $posicion = strpos($email, '@' . $dominio);
        if ($posicion !== false && $posicion === strlen($email) - strlen('@' . $dominio)) {
            return true;
        }
    }
    return false;
}
$dominiosPermitidos = ["gmail.com", "outlook.com"];

foreach ($emails as $email) {
    echo "</br>¿El correo '$email' pertenece a un dominio valido? " . (emailPermitido($email, $dominiosPermitidos) ? "Sí" : "No") . "</br>";
}
...
function reemplazarPalabra($texto, $buscar, $reemplazar) {
    $posicion = strpos($texto, $buscar);
    if ($posicion !== false) {
        $texto = substr_replace($texto, $reemplazar, $posicion, strlen($buscar));
    }
    return $texto;
}

$fraseOriginal = "PHP, un poderoso lenguaje de programacion";
$buscar = "poderoso";
$reemplazar = "versátil";
$fraseModificada = reemplazarPalabra($fraseOriginal, $buscar, $reemplazar);
